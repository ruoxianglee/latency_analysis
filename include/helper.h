...
void delay_some_time(double & delay_previous, int period)
{
    ...
    int delay_upper_now = 0;
    if(delay_previous <= period)
    {
        delay_upper_now = upper_;
    }
    else if(delay_previous > period && delay_previous <= period + upper_)
    {
        delay_upper_now = upper_ - (delay_previous - period);
    }
    else if(delay_previous > period + upper_)
    {
        delay_upper_now = 0;
    }

    double observed_delay = 0.;

    if (delay_upper_now != 0)
    {
        ...
        observed_delay = double(duration.count()) * microseconds::period::num / microseconds::period::den * Mstos;
    }
...
void delay_random_time(double & delay_previous, int period, int delay_upper_)
{
    ...
    if(delay_flag(gen))
    {
        ...
        else if(delay_previous > period && delay_previous <= period + delay_upper_)
        {
            delay_upper_now = delay_upper_ - (delay_previous - period);
        }
        else if(delay_previous > period + delay_upper_)
        {
            delay_upper_now = 0;
        }
